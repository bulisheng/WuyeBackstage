...
### 本轮新增
- 旧的操作员切换入口已经停用，后台现在统一走登录/退出模式，不再依赖顶部选择操作员。
...
### 第二阶段 P1 进展
- 统一工单底座已经开始落地，新增了 `tasks / task_logs / task_images`。
- 缴费底座已经落地，新增了 `bills / bill_items / payments / bill_reminders`。
- 云端 CloudBase MySQL 已经初始化账单相关表。
- `sxmini` 云函数已经同步到 CloudBase。
- 住户侧缴费入口已经从 `billing/*` 统一到 `user/billing/*`，会先走模块开关校验。
- 后台“缴费管理”页已经补了账单创建、删除、催缴。
- 静态站点已经重新发布。
- 后台现在支持初始化模式：当还没有管理员登录时，仍能通过 `bootstrap/communities` 看到小区列表，并直接新增首个管理员。
- 后台已经切换为登录/退出模式，不再要求先登录选择操作对象。
- 系统会自动种一个超级管理员，默认账号是 `super_admin`，用于管理其他管理员和权限。
- 默认密码由环境变量控制，未配置时使用 `Admin@123456`。
...
### 优先级 3：继续收紧剩余后台目录
当前已经完成：
- `admin/user/list` 已独立为管理员管理列表
- 后台已切换为登录/退出模式，不再依赖顶部操作员目录
...
- 先把缴费的支付记录和催缴视图补完整。
- 再把报修系统接到统一工单底座。
- 后续投诉 / 服务单也要复用同一套 `tasks` / `task_logs`。
...
- 如果后续继续推进第二阶段 P1，记得同步更新 `shengxing-property-core/docs/AI理解文档.md`，让下一位 AI 先读到最新状态。
- 如果后台没有操作员，不要误判为数据丢失，先走初始化模式创建首个管理员。
- 如果你要新增管理员或调整权限，先用超级管理员登录，不要再依赖旧的“操作员选择器”。
