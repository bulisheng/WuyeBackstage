	...
	<div class="layout">
		...
		<main v-if="isLoggedIn" class="main">
			...
			<div class="title-group">
				...
				<div class="toolbar">
					<div v-if="currentAdminInfo" class="admin-badge">
						<span>当前管理员</span>
						<strong>{{ currentAdminInfo.username }} · {{ currentAdminInfo.roleLabel }}</strong>
					</div>
					...
					<button @click="logoutAdmin">退出</button>
			...
			<section v-if="activeTab === 'permissions'" class="panel permission-panel">
				...
				<template v-if="activePermissionTab === 'admins'">
				...
				<template v-else-if="activePermissionTab === 'modules'">
					<div class="permission-grid">
						<div class="permission-card span-2">
							<div class="panel-head compact">
								<h3>当前小区模块开关</h3>
								<span>{{ activeCommunity ? communityLabel(activeCommunity) : '请选择小区' }}</span>
							</div>
							<div v-if="!activeCommunity" class="empty-state">
								<div class="empty-title">请先在顶部选择一个小区</div>
								<span class="subtle">模块开关按小区保存，新建小区默认全部开启。</span>
							</div>
							<template v-else>
								<div class="module-actions">
									<span class="helper-text">当前状态：{{ currentCommunityModuleSummary }}</span>
									<div class="form-actions compact">
										<button class="primary" :disabled="!canAction('community:module:manage')" @click="restoreAllModules">全部开启</button>
										<button :disabled="!canAction('community:module:manage')" @click="batchUpdateModules(0)">全部关闭</button>
									</div>
								</div>
								<table class="spaced-table">
									<thead>
									...
									<tbody>
										<tr v-for="item in currentCommunityModules" :key="item.key">
											<td>
												<div class="matrix-community">
													<strong>{{ moduleDisplayLabel(item) }}</strong>
													<span>{{ item.key }}</span>
												</div>
											</td>
											<td>{{ moduleDisplayDescription(item) || '暂无说明' }}</td>
											<td>{{ item.group || '-' }}</td>
											<td>{{ item.authRequired ? '是' : '否' }}</td>
											<td><span class="status" :class="item.enabled ? 'approved' : 'disabled'">{{ moduleEnabledLabel(item) }}</span></td>
											<td class="actions">
												<button :disabled="!canAction('community:module:manage')" @click="toggleModule(item)">{{ item.enabled ? '关闭' : '开启' }}</button>
											</td>
										</tr>
									</tbody>
								</table>
			...
			<section v-if="activeTab === 'fees'" class="panel">
				...
				<div class="announcement-editor">
					<div class="form-grid">
						<label class="field">
							<span>账单号</span>
							<input v-model="feeForm.billNo" type="text" placeholder="不填则自动生成" />
						</label>
						<label class="field">
							<span>标题</span>
							<input v-model="feeForm.title" type="text" placeholder="例如 2026年4月物业费" />
						</label>
						<label class="field">
							<span>业主</span>
							<input v-model="feeForm.ownerName" type="text" placeholder="请输入业主姓名" />
						</label>
						<label class="field">
							<span>房屋</span>
							<input v-model="feeForm.house" type="text" placeholder="例如 1-2-301" />
						</label>
						<label class="field">
							<span>类型</span>
							<input v-model="feeForm.billType" type="text" placeholder="例如 物业费 / 停车费" />
						</label>
						<label class="field">
							<span>金额</span>
							<input v-model.number="feeForm.amount" type="number" min="0" step="0.01" />
						</label>
						<label class="field">
							<span>状态</span>
							<select v-model="feeForm.status">
						...
						<label class="field">
							<span>截止日期</span>
							<input v-model="feeForm.dueDate" type="date" />
						</label>
					</div>
					<div class="form-actions">
						<button class="primary" :disabled="!canAction('fee:manage')" @click="saveFee">{{ editingFeeId ? '保存账单' : '新增账单' }}</button>
						<button @click="resetFeeForm">重置</button>
					</div>
				</div>
				<table>
					<thead>
					...
					<tbody>
						<tr v-for="item in fees" :key="item.id">
							...
							<td class="actions">
								<button :disabled="!canAction('fee:manage')" @click="editFee(item)">编辑</button>
								<button :disabled="!canAction('fee:remind')" @click="remindFee(item)">催缴</button>
								<button class="danger" :disabled="!canAction('fee:manage')" @click="removeFee(item)">删除</button>
							</td>
						</tr>
					</tbody>
				</table>
...
async function restoreAllModules() {
	const confirmed = window.confirm(`确认将「${communityLabel(activeCommunity.value)}」恢复为全部模块开启？`);
	if (!confirmed) return;
