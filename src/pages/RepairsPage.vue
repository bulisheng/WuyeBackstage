			...
			<tbody>
				<tr v-for="staff in workspace.repairStaff" :key="staff.id">
					...
					<td class="actions">
						...
						<button v-if="workspace.canAction('repair:assign')" class="danger" :disabled="!workspace.canAction('repair:assign')" @click="workspace.removeRepairStaff(staff)">删除</button>
					</td>
				</tr>
			</tbody>
		</table>
