	...
	<section class="page-section">
		<div class="panel-head">
			...
			<div class="form-actions compact">
				...
				<button class="primary" type="button" @click="openFaqModal()">新增常见问题</button>
			</div>
		</div>
		...
		<ModalDialog v-if="faqModalOpen" :title="faqModalTitle" subtitle="保存后会同步到在线客服自动回复" @close="closeFaqModal">
			<div class="form-grid">
...
async function save() {
	try {
		...
		window.alert('常见问题保存成功');
		reset();
		faqModalOpen.value = false;
		await loadList();
	} catch (err) {
		window.alert(err.message || '常见问题保存失败');
	}
}
