...
function callCloud(route, params = {}, options) {
	...
	return new Promise(function (resolve, reject) {
		...
		wx.cloud.callFunction({
			...
			fail: function (err) {
				if (hint) {
					console.error('[SXWY] wx.cloud.callFunction failed', err)
					let errMsg = err && err.errMsg ? err.errMsg : '';
					let errCode = err && err.errCode ? err.errCode : '';

					if (errMsg.includes('-501000') && errMsg.includes('Environment not found')) {
						wx.showModal({
							title: '',
						...
						(errMsg.includes('FunctionName') || errMsg.includes('function') || errMsg.includes('not found') || errMsg.includes('does not exist'))
					) {
						wx.showModal({
							title: '',
					...
					} else if (errMsg.includes('-501000') && errMsg.includes('performed in the current function state')) {
						wx.showModal({
							title: '',
						...
						wx.showModal({
							title: '',
							content: `云函数调用失败${errCode ? '：' + errCode : ''}${errMsg ? '\n' + errMsg : ''}`,
							showCancel: false
						});
					}
				}
