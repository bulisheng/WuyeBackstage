...
class PassportService extends BaseProjectService {
	...
	async register(userId, {
		...
		await this._syncUserHouse(userId, mobile, name, forms);
	...
	async getMyDetail(userId) {
		...
		let houses = await UserHouseModel.getAll({
			USER_MINI_OPENID: userId
		}, '*', { USERHOUSE_ADD_TIME: 'desc' });
		user.USER_HOUSES = houses || [];
		user.USER_HOUSE = houses && houses[0] ? houses[0].HOUSE_TEXT : '';
		return user;
	}
	...
	async _syncUserHouse(userId, mobile, name, forms) {
		let houseText = dataUtil.getValByForm(forms || [], 'house') || dataUtil.getValByForm(forms || [], 'house', '楼栋房号') || '';
		let communityName = dataUtil.getValByForm(forms || [], 'community') || '';
		if (!houseText) return;

		await UserHouseModel.insertOrUpdate({
			USER_MINI_OPENID: userId
		}, {
			USER_MINI_OPENID: userId,
			USER_MOBILE: mobile,
